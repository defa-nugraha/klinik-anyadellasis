            ...
            <div class="d-flex justify-content-between">
                ...
                @if($statusRekamMedis)
                    @if($statusRekamMedis->status == 'antrian')
                        <a href="{{route('nurse.rekam_medis.update-status').'?rekam_medis='.encryptStr($statusRekamMedis->id).'&status=pemeriksaan'}}" class="btn btn-primary">Lanjutkan ke dokter <i class="fas fa-chevron-right"></i></a>
                    @elseif($statusRekamMedis->status == 'pemeriksaan')
                        <a href="{{route('nurse.rekam_medis.update-status').'?rekam_medis='.encryptStr($statusRekamMedis->id).'&status=di apotek'}}" class="btn btn-primary">Selesaikan pemeriksaan & perawatan <i class="fas fa-chevron-right"></i></a>
                    @elseif($statusRekamMedis->status == 'di apotek')
                        <a href="{{route('nurse.rekam_medis.update-status').'?rekam_medis='.encryptStr($statusRekamMedis->id).'&status=selesai'}}" class="btn btn-primary">Selesaikan rekam medis ini <i class="fas fa-chevron-right"></i></a>
                    @endif
                @endif
            </div>
            <x-table>
                    ...
                    @foreach ($rekamMedis as $item)
                        <tr>
                            ...
                            <td>
                                                ...
                                                <div class="modal-dialog modal-xl">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body">
                                                            <form action="{{route('nurse.pemeriksaan.createOrUpdate')}}" method="post" enctype="multipart/form-data">
                                                                @csrf
                                            ...
                                            <div class="modal fade" id="modalDiagnosa{{$item->id}}" tabindex="-1" aria-labelledby="modalDiagnosa{{$item->id}}Label" aria-hidden="true">
                                                <div class="modal-dialog modal-xl">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body">
                                                            <form action="{{route('nurse.diagnosa.createOrUpdate')}}" method="post" enctype="multipart/form-data">
                                                                @csrf
                                                                <input type="hidden" name="rekam_medis" value="{{encryptStr($item->id)}}">
                                            ...
                                            <div class="modal fade" id="modalTindakan{{$item->id}}" tabindex="-1" aria-labelledby="modalTindakan{{$item->id}}Label" aria-hidden="true">
                                                <div class="modal-dialog modal-xl">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body">
                                                            <form action="{{route('nurse.tindakan.createOrUpdate')}}" method="post" enctype="multipart/form-data">
                                                                @csrf
                                                                <input type="hidden" name="rekam_medis" value="{{encryptStr($item->id)}}">
                                ...
                                @else  
                                    <a href="{{route('nurse.obat-keluar.detail', encryptStr($item->id))}}" class="btn btn-sm btn-success">
