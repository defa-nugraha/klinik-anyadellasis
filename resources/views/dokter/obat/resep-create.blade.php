        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex justify-content-between">
                    ...
                    <a href="{{route('dokter.rekam_medis.update-status').'?rekam_medis='.encryptStr($rekamMedis->id).'&status=selesai'}}" class="btn btn-success mb-3">
                        Selesaikan tanpa pemberian obat <i class="fa fa-check-circle fs-6"></i> 
                    </a>
                </div>

                <form action="{{route('dokter.obat.storeResep')}}" method="post">
                    <x-table>
                        <thead>
                            <th>Kode</th>
                            <th>Nama</th>
